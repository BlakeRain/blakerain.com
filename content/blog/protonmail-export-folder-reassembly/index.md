...
I would usually use [fetchmail] to get email from one IMAP account into another, but ProtonMail does
not support IMAP access, so that was not an option. There may be other options for exporting from
ProtonMail, but my cursory search did not turn up anything useful.
{% endcall %}
