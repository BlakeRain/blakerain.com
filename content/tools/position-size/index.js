...
function Label(props) {
  ...
  return html`
    <div class=${klass}>
      <label class="text-sm font-medium"> ${props.title} </label>
      ${props.children}
    </div>
  `;
}
...
function Number(props) {
  const [inputValue, setInputValue] = useState(
    formatNumber(props.value, false, props.places, null, null),
  );
  ...
  const rendered = focused
    ? inputValue
    : formatNumber(props.value, props.thousands, props.places, props.prefix, props.suffix);
...
function Tooltip(props) {
  ...
  return html`
    <div
      class="relative inline-block cursor-pointer mx-2"
      onmouseover=${onmouseover}
      onmouseout=${onmouseout}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        data-license="From https://github.com/twbs/icons - Licensed under MIT"
        width="1em"
        height="1em"
        fill="currentColor"
        viewBox="0 0 16 16"
...
function TableRow(props) {
  ...
  const number = formatNumber(
    ...
    true,
    props.places,
    props.currency ? CURRENCY_SYMBOLS[props.currency] : null,
    props.suffix,
  );
  const tooltip = props.children
    ? html`<${Tooltip} position=${props.tooltipPosition}>${props.children}</${Tooltip}>`
    : null;

  return html`
    <tr class=${props.error ? "text-red-500" : ""}>
      ${title}
      <td class="text-right">${number} ${tooltip}</td>
    </tr>
  `;
}
...
function AccountInfo() {
  ...
  const onPositionRiskChange = (value) =>
    dispatch({ type: "setPositionRisk", payload: value / 100 });
  ...
  const pcRow = pc
    ? html`
        <tr>
          ...
          <td class="text-right">
            ${formatNumber(
              account.exchangeRates[position.positionCurrency] || 0,
              false,
              account.places,
              CURRENCY_SYMBOLS[position.positionCurrency],
              null,
            )}
          </td>
        </tr>
        ...
        <tr>
          ...
          <td class="text-right">
            ${formatNumber(
              ...
              false,
              account.places,
              CURRENCY_SYMBOLS[position.quoteCurrency],
              null,
            )}
          </td>
        </tr>
  ...
  <${Panel} title="Account Information" class="grid grid-cols-1 sm:grid-cols-2 items-center gap-4">
    <${Label} title="Account Currency">
      <${CurrencySelect} value=${account.currency} onchange=${(currency) =>
        dispatch({ type: "setCurrency", payload: currency })} />
    </${Label}>
    <${Label} title="Account Value">
      <${Number} thousands prefix=${CURRENCY_SYMBOLS[account.currency]} places=${
        account.places
      } value=${account.amount} onchange=${onAmountChange} />
    </${Label}>
    <${Label} title="Margin Risk">
      <${Number} value=${
        account.marginRisk * 100
      } places=${0} suffix="%" onchange=${onMarginRiskChange} />
    </${Label}>
    <${Label} title="Position Risk">
      <${Number} value=${
        account.positionRisk * 100
      } places=${0} suffix="%" onchange=${onPositionRiskChange} />
    </${Label}>
...
function PositionInfo() {
  ...
  const onPositionCurrencyChange = (currency) =>
    dispatch({ type: "setPositionCurrency", payload: currency });
  const onQuoteCurrencyChange = (currency) =>
    dispatch({ type: "setQuoteCurrency", payload: currency });
  ...
  const onStopLossToggle = (value) =>
    dispatch({ type: "setStopLoss", payload: value ? position.openPrice : null });
  const onStopLossChange = (value) => dispatch({ type: "setStopLoss", payload: value });
  const onTakeProfitToggle = (value) =>
    dispatch({ type: "setTakeProfit", payload: value ? position.openPrice : null });
  const onTakeProfitChange = (value) => dispatch({ type: "setTakeProfit", payload: value });

  const onStopLossDistanceChange = (value) =>
    dispatch({
      type: "setStopLoss",
      payload:
        position.direction === "BUY" ? position.openPrice - value : position.openPrice + value,
    });

  const onTakeProfitDistanceChange = (value) =>
    dispatch({
      type: "setTakeProfit",
      payload:
        position.direction === "BUY" ? position.openPrice + value : position.openPrice - value,
  ...
  const positionLeverage =
    position.margin !== 0
      ? ` (${formatNumber(1 / position.margin, false, 0, null, null)}x leverage)`
      : "";
    ...
    <${Panel} title="Position Information" class="grid grid-cols-1 sm:grid-cols-2 items-center gap-4">
      <div class="grid grid-cols-2 gap-4">
        <${Label} title="Position">
          ...
          } onchange=${onPositionCurrencyChange} />
        </${Label}>
        <${Label} title="Quote">
      ...
      <${Label} title=${positionMargin}>
        <${Number} value=${
          position.margin * 100
        } places=${2} suffix="%" onchange=${onMarginChange} />
      </${Label}>
      ...
      <${Label} title="Open Price">
        <${Number} value=${position.openPrice} thousands places=${account.places} prefix=${
          CURRENCY_SYMBOLS[position.quoteCurrency]
      ...
      <${Label} title="Quantity" class="col-start-1">
        <div class="flex flex-row gap-4">
          ...
          <${Number} class="w-full" value=${
            position.quantity || 0
          } thousands places=${4} suffix=" units" disabled=${
            position.quantity === null
          } onchange=${onQuantityChange} />
        </div>
      </${Label}>
      <div class="grid grid-flow-col justify-stretch gap-2 pt-7">
        <button type="button" class="button" onclick=${onAffordableClick} disabled=${
          position.quantity === null || position.openPrice === 0
        }>Affordable Quantity</button>
        <button type="button" class="button" onclick=${onStopLossClick} disabled=${
          position.stopLoss === null || stopLossDistance === 0
        }>Stop Loss Quantity</button>
      </div>
      <${Label} title="Stop Loss">
        <div class="flex flex-row gap-4">
          <${Toggle} value=${position.stopLoss !== null} onchange=${onStopLossToggle} />
          <${Number} class="w-full" value=${position.stopLoss || 0} thousands places=${
            account.places
          } prefix=${CURRENCY_SYMBOLS[position.quoteCurrency]} disabled=${
            position.stopLoss === null
          } onchange=${onStopLossChange} />
        </div>
      </${Label}>
      <${Label} title="Stop Loss Distance">
        <${Number} value=${stopLossDistance} thousands places=${account.places} prefix=${
          CURRENCY_SYMBOLS[position.quoteCurrency]
        } disabled=${position.stopLoss === null} onchange=${onStopLossDistanceChange} />
      </${Label}>
      <${Label} title="Take Profit">
        <div class="flex flex-row gap-4">
          <${Toggle} value=${position.takeProfit !== null} onchange=${onTakeProfitToggle} />
          <${Number} class="w-full" value=${position.takeProfit || 0} thousands places=${
            account.places
          } prefix=${CURRENCY_SYMBOLS[position.quoteCurrency]} disabled=${
            position.takeProfit === null
      ...
      <${Label} title="Take Profit Distance">
        <${Number} value=${takeProfitDistance} thousands places=${account.places} prefix=${
          CURRENCY_SYMBOLS[position.quoteCurrency]
        } disabled=${position.takeProfit === null} onchange=${onTakeProfitDistanceChange} />
      </${Label}>
    </${Panel}>
  `;
}
...
function ReportPositionSize() {
  ...
  const actualTable = actual
    ? html`
    <${TableRow} title="Actual Quantity" value=${position.quantity} places=${2} suffix=" units">
      Quantity entered into the position form.
    </${TableRow}>
    <${TableRow} title="Actual Cost" value=${position.quantity * position.openPrice} places=${
      account.places
    } currency=${position.quoteCurrency}>
      Actual const of opening a position of ${quantityFormat} units at ${openPriceFormat}.
    </${TableRow}>
    <${TableRow} title="Required Margin" value=${actual.costQuote} places=${
      account.places
    } currency=${position.quoteCurrency}>
      Amount required at ${marginFormat} position margin (${formatNumber(
        1 / (position.margin === 0 ? 1 : position.margin),
        false,
        0,
        null,
        null,
    ...
    ${
      pq &&
      html`<${TableRow} value=${actual.costPosition} places=${account.places} currency=${position.positionCurrency}>
      Amount required at ${marginFormat} margin, converted into the position currency.
    </${TableRow}`
    }
    ${
      ap &&
      html`<${TableRow} value=${actual.cost} places=${account.places} currency=${account.currency}>
      Amount required at ${marginFormat} margin, converted into the account currency.
    </${TableRow}`
    }
    <${TableRow} title="Committed Account" value=${
      actual.margin * 100
    } places=${2} suffix="%" error=${excessRisk}>
      ...
      html`
        <tr class="text-red-500">
          <th colspan="2" class="text-left font-normal">
            Actual quantity of ${quantityFormat} exceeds account margin risk of
            ${formatNumber(account.marginRisk * 100, true, 2, null, "%")} by
            ${formatNumber(
  ...
  return html`
    <${Panel} title="Position Size Information">
      <${Table}>
        <${TableRow} title="Available Account" value=${available} places=${
          account.places
        ...
        ${
          ap &&
          html`<${TableRow} value=${availablePosition} places=${account.places} currency=${position.positionCurrency}>
          Amount of account available under margin risk in the position currency.
        </${TableRow}>`
        }
        ${
          pq &&
          html`<${TableRow} value=${availableQuote} places=${account.places} currency=${position.quoteCurrency}>
          Amount of account available under margin risk in the quote currency.
        </${TableRow}>`
        }
        <${TableRow} title="Available Margin" value=${margin} places=${account.places} currency=${
          account.currency
        }>
        ...
        ${
          ap &&
          html`<${TableRow} value=${marginPosition} places=${account.places} currency=${position.positionCurrency}>
          Available amount with a ${marginFormat} position margin converted to the position currency.
        </${TableRow}>`
        }
        ${
          pq &&
          html`<${TableRow} value=${marginQuote} places=${account.places} currency=${position.quoteCurrency}>
...
function ReportStopLoss() {
  ...
  const { available, availablePosition, availableQuote, distance, actual } = computeStopLoss(
    account,
    position,
    quantity,
  );
  ...
  const maximumStopLoss =
    position.direction === "BUY" ? position.openPrice - distance : position.openPrice + distance;
  ...
  if (actual) {
    ...
    actualTable = html`
      <tr />
      <${TableRow} title="Actual Distance" value=${distance} places=${account.places} currency=${
        position.positionCurrency
      }>
        The distance provided in the position form.
      </${TableRow}>
      <${TableRow} title="Actual Loss" value=${loss} places=${account.places} currency=${
        account.currency
      }>
      ...
      <${TableRow} title="Actual Risk" error=${excessRisk} value=${
        risk * 100
      } places=${2} suffix="%">
      ...
      ${
        ...
        html`
          <tr class="text-red-500">
            <th colspan="2" class="text-left font-normal">
              Actual stop loss of ${stopLossFormat} exceeds account position risk of
              ${positionRiskFormat} by
              ${formatNumber(loss - available, true, account.places, ac, null)}.
            </th>
          </tr>
        `
      }
  ...
  return html`
    <${Panel} title="Stop Loss Position" class="flex flex-col gap-4">
      <${Table}>
        ...
        <${TableRow} title="Maximum Stop Loss Price Distance" value=${distance} places=${
          account.places
        } currency=${position.positionCurrency}>
        ...
        <${TableRow} title="Maximum Stop Loss" value=${maximumStopLoss} places=${
          account.places
        } currency=${position.positionCurrency}>
      ...
      <p class="text-neutral-500 text-sm">
        This panel shows the maximum available stop loss, given the ${
          position.quantity ? "specified" : "calculated"
        } position size of ${quantityFormat}, and the account position risk.
      </p>
    </${Panel}>
  `;
}
...
function ReportTakeProfit() {
  ...
  const realizedAccount =
    realized / (position.conversion * (account.exchangeRates[position.positionCurrency] || 1));

  return html`
    <${Panel} title="Take Profit" class="flex flex-col gap-4">
      <${Table}>
        <${TableRow} title="Take Profit" value=${position.takeProfit} places=${
          account.places
        } currency=${position.positionCurrency}>
          Take profit entered into the position form.
        </${TableRow}>
        <${TableRow} title="Take Profit Distance" value=${takeProfitDistance} places=${
          account.places
        } currency=${position.positionCurrency}>
        ...
        ${
          position.stopLoss !== null &&
          html`<${TableRow} title="Ratio to Stop Loss" error=${stopLossRatio < 2} value=${
            stopLossRatio * 100
          } places=${account.places} suffix="%">
          The ratio of the take profit distance to the stop loss distance.
        </${TableRow}>
        ${
          stopLossRatio < 2 &&
          html`<tr class="text-red-500">
            <th colspan="2" class="text-left font-normal">
              A profit/loss ratio of ${formatNumber(stopLossRatio * 100, true, 0, null, "%")} is
              below the recommended minimum of 2:1
            </th>
          </tr>`
        }`
        }
        <${TableRow} title="Total Profit" value=${realized} places=${account.places} currency=${
          position.positionCurrency
        }>
          Total realized profit if closing a position of ${quantityFormat} at ${takeProfitFormat}.
        </${TableRow}>
        <${TableRow} title="Realized Profit" value=${realizedAccount} places=${
          account.places
        } currency=${account.currency}>
...
function ReportPlannedStopLoss() {
  ...
  const { available, availablePosition, availableQuote, distance, affordable, margin } =
    computeStopLossQuantity(account, position);

  let marginTable = null;
  if (distance !== 0) {
    marginTable = html`
      <${TableRow} title="Required Margin" value=${margin} places=${account.places} currency=${
        account.currency
      }>
  ...
  return html`
    <${Panel} title="Planned Stop Loss Maximum" skip class="flex flex-col gap-4">
      <${Table}>
        ...
        <${TableRow} title="Stop Loss" value=${position.stopLoss || 0} places=${
          account.places
        } currency=${position.positionCurrency}>
          Stop loss entered into the position form.
        </${TableRow}>
        <${TableRow} title="Stop Loss Distance" value=${distance} places=${
          account.places
        } currency=${position.positionCurrency}>
        ...
        <${TableRow} title="Available Quantity" value=${affordable} places=${2} suffix=" units">
          The position size that can be taken at an open price of ${formatNumber(
            position.openPrice,
            true,
            account.places,
            qc,
            null,
          )}, given an account position risk of ${positionRiskFormat}.
        </${TableRow}>
      </${Table}>
      <p class="text-neutral-500 text-sm">
