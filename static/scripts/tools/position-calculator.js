...
class PositionMarginLabelElement extends HTMLElement {
  ...
  _updateLabel() {
    ...
    if (margin === 0.0) {
      ...
      this._label.innerText = `Position Margin (${(1 / margin).toFixed(2)}x leverage)`;
    }
  }
}
...
function buildReportRows(container, reportRows) {
  ...
  for (const row of reportRows) {
    ...
    if (row.hasHelp) {
      ...
      helpContainer.classList.add("help");

      const icon = HELP_ICON.content.cloneNode(true);

      help = document.createElement("div");
      help.classList.add("tooltip", "right");
      if (!row.mutatingHelp) {
...
function updateReportRows(rowElements, reportRows, report) {
  for (let i = 0; i < reportRows.length; i++) {
    ...
    if (row.mutatingError) {
      if (row.error(report)) {
        elements.row.classList.add("danger");
        if (elements.description) {
          elements.description.classList.add("danger");
        }
      } else {
        elements.row.classList.remove("danger");
        if (elements.description) {
          elements.description.classList.remove("danger");
        }
      }
    }
...
class ReportElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._container = this.querySelector(".report-panel");
    if (!this._container) {
      throw new Error("<report-element> must contain a .report-panel element");
    }
