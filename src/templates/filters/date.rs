    ...
    fn try_from(value: &Value) -> Result<Self, Self::Error> {
        if let Some(value) = value.as_str() {
            ...
            fn serde_error(err: serde::de::value::Error) -> Error {
                Error::new(ErrorKind::InvalidOperation, "Not a valid date or datetime")
                    .with_source(err)
            }
...
pub fn datetime(value: &Value, kwargs: Kwargs) -> Result<String, Error> {
    let datetime = match ParsedDate::try_from(value)? {
        ParsedDate::Date(date) => date.midnight().assume_utc(),
        ParsedDate::DateTime(datetime) => datetime,
    };
    ...
    match kwargs.get::<Option<&str>>("format")? {
        ...
        Some(format) => {
            if format == "rfc2822" {
                ...
                let format =
                    time::format_description::parse_borrowed::<3>(format).map_err(|err| {
...
pub fn date(value: &Value, kwargs: Kwargs) -> Result<String, Error> {
    ...
    match kwargs.get::<Option<&str>>("format")? {
        ...
        Some(format) => {
            if format == "rfc2822" {
                ...
                let format =
                    time::format_description::parse_borrowed::<3>(format).map_err(|err| {
                        Error::new(ErrorKind::InvalidOperation, "invalid date format")
                            .with_source(err)
                    })?;
