...
struct Args {
    ...
    /// Parse the input as YAML (as opposed to JSON)
    #[arg(long, env)]
    pub yaml: bool,
    ...
    /// The output path to write the HTML
    #[arg(short, long, env)]
    ...
    /// Path to the template to use for rendering
    pub template: String,
}
...
fn main() -> anyhow::Result<()> {
    ...
    let page = if args.yaml {
        ...
        parse_yaml(&input).context("failed to parse input as YAML")?
    } else {
        serde_json::from_reader::<_, serde_json::Value>(std::io::stdin())
